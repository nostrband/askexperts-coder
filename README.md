...
- `askexperts` package installed globally (`npm install -g askexperts`)
...
```bash
askexperts-coder symbols ./my-project
```

### 2. Generate Documentation
Generate AI-powered documentation for all symbols:
```bash
askexperts-coder generate ./my-project --nwc "nostr+walletconnect://..."
```

### 3. Prepare for Import
Convert the generated docs to docstore format:
```bash
askexperts-coder prepare ./my-project --dir ./prepared-docs
```
...
**Usage:**
```bash
askexperts-coder generate ./my-project
```
...
askexperts-coder generate ./my-project --debug --threads 4 --nwc "nostr+walletconnect://..."

# Continue interrupted generation
askexperts-coder generate ./my-project --continue
```
...
**Usage:**
```bash
askexperts-coder prepare ./my-project --output docs.txt
```
...
askexperts-coder prepare ./my-project --output ./docs/combined-docs.txt

# Create individual JSON files for each symbol
askexperts-coder prepare ./my-project --dir ./docs/symbols/
```
...
askexperts-coder symbols ./my-project --debug
```
...
askexperts-coder symbols ./my-typescript-project

# 2. Generate documentation (requires NWC for payments)
askexperts-coder generate ./my-typescript-project --nwc "nostr+walletconnect://..."

# 3. Prepare documentation for docstore import
askexperts-coder prepare ./my-typescript-project --dir ./prepared-docs
...
askexperts-coder generate ./my-project --threads 8 --debug
```
...
```bash
askexperts-coder generate ./my-project --debug
```
